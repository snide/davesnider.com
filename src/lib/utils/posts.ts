...
export async function getPostSummaries(): Promise<PostSummary[]> {
	const files = import.meta.glob<{ metadata: PostMetadata }>(
	...
	return posts.sort(
		(a, b) => new Date(b.metadata.pubDate).getTime() - new Date(a.metadata.pubDate).getTime()
	);
}
...
export async function getPostBySlug(slug: string): Promise<Post | undefined> {
	...
	const entry = Object.entries(files).find(([path]) =>
		path.split('/').pop()?.replace('.svx', '') === slug
	);
